...
public class TextureScale
{
	...
	{
		Texture2D tmp = tex;
		texColors = tmp.GetPixels();
		newColors = new Color[newWidth * newHeight];
		if (useBilinear)
		{
			ratioX = 1.0f / ((float)newWidth / (tmp.width-1));
			ratioY = 1.0f / ((float)newHeight / (tmp.height-1));
		}
		else {
			ratioX = ((float)tmp.width) / newWidth;
			ratioY = ((float)tmp.height) / newHeight;
		}
		w = tmp.width;
		w2 = newWidth;
